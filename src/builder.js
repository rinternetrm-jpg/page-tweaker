...
(function () {
  if (window.__ptBuilderActive) return;
  ...
  // === Original-Seite sichern & ersetzen ===
  const originalHtml = document.documentElement.innerHTML;
  const originalTitle = document.title;

  // === Komplett neuen Body aufbauen ===
  document.head.innerHTML = `
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>PageTweaker — ${originalTitle.replace(/</g, '&lt;')}</title>
    <style>${getBuilderCSS()}</style>
  `;

  document.body.innerHTML = '';
  document.body.className = 'pt-builder-body';
  ...
  function getDefaultOptions(type) {
    const opts = {
      'recommendations': { columns: 1, thumbnailWidth: 168, maxItems: 10 },
      'comments': { count: 5 },
    };
    return opts[type] || {};
  }
  ...
  function setupItemResize(wrapper, item, handle) {
    handle.addEventListener('mousedown', (e) => {
      ...
      const onMove = (e) => {
        item.w = Math.max(100, startW + (e.clientX - startX));
        item.h = Math.max(50, startH + (e.clientY - startY));
        wrapper.style.width = item.w + 'px';

        // Re-render mockup for video player (aspect ratio)
        if (item.type === 'video-player') {
          const content = wrapper.querySelector('.pt-item-content');
          content.innerHTML = '';
          content.appendChild(renderer.renderVideoPlayer(item.data, item.w, item.h));
        }
      };
  ...
  function updatePaletteStatus() {
    const placed = new Set(placedItems.map(i => i.type));
    palette.querySelectorAll('[data-status-for]').forEach(el => {
      el.textContent = placed.has(el.dataset.statusFor) ? '✓' : '';
    });
  }
  ...
  function exitBuilder() {
    ...
    // Reload to restore YouTube's JS
